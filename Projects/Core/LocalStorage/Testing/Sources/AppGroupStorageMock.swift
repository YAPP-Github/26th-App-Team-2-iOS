...
public final actor MockAppGroupStorage {
    @MainActor
...
@MainActor extension MockAppGroupStorage: AppGroupStorageProtocol {
    ...
    public func getAppGroupEntity(groupID: Int) async throws -> AppGroupEntity {
        ...
        guard let appGroup = try? await context.fetch(descriptor).first else {
